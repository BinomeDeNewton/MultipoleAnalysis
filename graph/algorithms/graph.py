...
from mathematics.signalflow_algorithms.common.json_dict import JSONDict
import uuid
...
class Branch(GraphMember, JSONDict):
    ...
    def __str__(self):
        return f"Branch from {self.start.label} to {self.end.label} with weight {self.weight}"

    def __repr__(self):
        return f"Branch(start={self.start.label}, end={self.end.label}, weight={self.weight})"
...
class Node(GraphMember, JSONDict):
    def __init__(self, graph: Graph, label: str = "", **kwargs):
        super().__init__(graph, **kwargs)
        self.__ingoing: Set[Branch] = set()
        self.__outgoing: Set[Branch] = set()
        self.label = label  # Ajoutez le nouvel attribut label
        # Add self to graph
        graph.add_node(self)
